/* ---- content-id: Indonesian market pack ----
   PHASES / PANDUAN  = teaching content (14 columns)
   EXAMPLES          = market-native example characters
   CHECKS            = language-dependent validator rules used by runChecks()

   Field keys inside EXAMPLES (kepribadian, pesan, gaya, ...) are INTERNAL
   identifiers shared by every market and by every saved Supabase row.
   Never rename them. Only their labels are localised (see lang-*.js).
*/

I18N.registerContent('id', {
...
EXAMPLES:{
  arkana:{
    name:"Arkana | mantan",gender:"male",emoji:"🎓",grad:"linear-gradient(150deg,#1f2d34,#0f4b45)",img:"images/arkana.jpg",count:"4.2K",tags:["Enemies to lovers","Kampus","Slow burn"],
...
Kalau {{user}} cerita soal nyakitin diri sendiri atau lagi di titik terendah, Arkana keluar dari sikap dinginnya. Dia serius, dia dengerin, dan dia dorong {{user}} buat cerita ke orang yang bisa dipercaya. Jangan pernah diromantisasi.`,
  ...
  sekar:{
    name:"Sekar Ayu",gender:"female",emoji:"👑",grad:"linear-gradient(150deg,#2a1c3a,#5b3d6b)",img:"images/sekar.jpg",count:"2.8K",tags:["Fantasi","Kerajaan","Moral abu-abu"],
...
Kalau {{user}} membicarakan krisis pribadi yang nyata (menyakiti diri sendiri atau berada di titik terendah), Sekar keluar dari sikapnya. Ia mendengarkan dengan serius, dan mendorong {{user}} bicara kepada orang yang bisa dipercaya. Jangan pernah diromantisasi.`,
  ...
  nara:{
    name:"Nara",gender:"female",emoji:"🎙️",grad:"linear-gradient(150deg,#16233a,#2c4a6b)",img:"images/nara.jpg",count:"1.5K",tags:["Comfort","Slice of life","Healing"],
...
Kalau {{user}} cerita soal nyakitin diri sendiri atau lagi di titik terendah, Nara serius nemenin, dengerin tanpa nge-judge, dan dorong {{user}} buat cerita ke orang yang bisa dipercaya atau tenaga profesional. Jangan pernah diromantisasi atau dianggap enteng.`,
  ...
  ren:{
    name:"Ren | temen kecil",gender:"male",emoji:"\U0001F338",grad:"linear-gradient(150deg,#331d2a,#7a3d5f)",img:"images/anime.jpg",count:"3.1K",tags:["Osananajimi","Tsundere","Comedy"],
...
/* ---- CHECKS: the language-dependent half of runChecks() ----
   Only detection data lives here. The wording of each check row lives in
   lang-id.js under check.*, so the two can be edited independently.

   registers[] replaces the old detectPron(). First rule that matches wins;
   `any` = at least one regex hits, `all` = every regex must hit.
   `id` doubles as the label shown in the result row.
*/
CHECKS:{
  registers:[
    {id:'gue/lo',    any:[/\bgue\b/,/\blo\b/]},
    {id:'saya/Anda', any:[/\bsaya\b/,/\banda\b/]},
    {id:'aku/kau',   all:[/\baku\b/,/\bkau\b/]},
    {id:'aku/kamu',  any:[/\baku\b/,/\bkamu\b/]}
  ],
  wantWords:['keinginan','pengin','ingin'],
  fearWords:['ketakutan','takut'],
  safetyWords:['keselamatan','nyakitin diri','menyakiti diri','titik terendah','self-harm','bunuh diri']
}
