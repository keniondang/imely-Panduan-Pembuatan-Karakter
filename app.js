...
function buildExampleGrid(){
  ...
  ['arkana','sekar','nara'].forEach(function(key){
    ...
    card.innerHTML=
      '<div class="ex-thumb" style="background:'+d.grad+'"><span class="em">'+d.emoji+'</span>'+
...
function renderEtalaseCard(d){
  ...
  var h='<div class="et-card"><div class="et-hero" style="background:'+grad+'">'+(d.emoji?'<span class="et-emoji">'+d.emoji+'</span>':'')+
  ...
  if(d.pesan)h+='<div class="pv-sec"><p class="lbl">Pesan pertama:</p><div class="pv-bubble"><span class="ava" style="background:'+grad+'">'+(d.emoji||'')+'</span><div class="msg">'+escHtml(d.pesan)+'</div></div></div>';
  h+='</div></div>';return h;
}
function cekKarakter(){
  ...
  var html=renderEtalaseCard({name:v.name,tagline:v.tagline,tags:(v.hashtags||[]).map(function(t){return '#'+t;}),
    catatan:v.catatan,info:v.info,bio:v.bio,pesan:v.pesan,grad:thm?thm.grad:null,emoji:thm?thm.emoji:''});
